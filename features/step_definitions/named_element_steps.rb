...
Then /^the ([^\"]*) field should contain ([^\"].*)$/ do |named_element, named_value|
  field = selector_for(named_element)
  value = selector_for(named_value)
...
Then 'I should see the $named_element to $url as a new window' do |named_element, url|
  within(selector_for(named_element)) do
    page.should have_css(%{a.open-facebook-share[href='#{facebook_share_url_for(path_to(url))}']})
  end
end

Then 'the metadata for "$property" should be the $named_element' do |property, named_element|
  page.should have_css("meta[property*='og:#{property}'][content*='#{selector_for(named_element)}']")
end

Then /^I can see a link to (.*) within (.*)$/ do |page_name, page_scope|
  scope = selector_for(page_scope)
  within(scope) do
    page.should have_css("a[href='#{path_to(page_name)}']")
  end
end

Then %{I should see the button "$button" within $page_scope} do |button, page_scope|
  within(selector_for(page_scope)) do
    page.should have_button(button)
  end
end

Then %{the $named_element link should be disabled} do |named_element|
  page.should have_css("#{selector_for(named_element)}.disabled")
end

When /^I fill in "([^"]+)" field with a string of length (\d+)$/ do |field_name,length|
  page.should have_css(selector_for(field_name))
  string = 'a' * length.to_i

  evaluate_script(%{
    var $answer = $('#{selector_for(field_name)}');
    $answer.trigger("focus").val("#{string}");
    $answer.trigger(jQuery.Event("keyup")).trigger(jQuery.Event("keydown")).trigger(jQuery.Event("keypress"));
  })
end

Then /^the length of the "([^"]+)" field should be (\d+)$/ do |field_name,length|
  evaluate_script("$('#{selector_for(field_name)}').val().length").should == length.to_i
end
